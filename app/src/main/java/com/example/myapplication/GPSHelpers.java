...
public class GPSHelpers {
    ...
    public static void chekGPS(FragmentActivity context, LocationManager locationManager, LocationListener locationListener) {
        if (!checkGPSPerms(context)) {
            Toast.makeText(context, context.getString(R.string.access_to_gps), Toast.LENGTH_LONG).show();
        }
        try {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.P) {
                if (!locationManager.isLocationEnabled()) {
                    Toast.makeText(context, context.getString(R.string.turn_upp_gps), Toast.LENGTH_SHORT).show();
                } else {
